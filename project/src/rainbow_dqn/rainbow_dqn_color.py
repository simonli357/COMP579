...
def run_experiment(cfg: dict,
                  ...
                  wrapper_cls=PreprocessFrame,
                  wrapper_kwargs=None) -> np.ndarray:
    ...
    ch0 = env.observation_space.shape[-1]  # 1 or 3
    net = build_rainbow(ch0 * 4, env.action_space.n,
                        noisy=cfg["noisy"],
                        dueling=True,
                        num_atoms=51)
...
if __name__ == "__main__":
    ...
    rewards_gray = run_experiment(cfg,
                                  Path("experiments/full_rainbow_gray"),
                                  wrapper_cls=PreprocessFrame,
                                  wrapper_kwargs={"grayscale": True})

    # 2) Red‑tint variant
    rewards_tint = run_experiment(cfg,
                                  Path("experiments/full_rainbow_tint"),
                                  wrapper_cls=ColorPreprocessFrame,
                                  wrapper_kwargs={"tint": (1.0, 0.0, 0.0)})

    # 3) Plot
    plt.plot(rewards_gray, label="Grayscale")
    plt.plot(rewards_tint, label="Red Tint")
    plt.xlabel("Episode")
    plt.ylabel("Episode Reward")
    plt.legend()
    plt.title("MsPacman: Grayscale vs. Red‑tint, 50k frames")
    plt.tight_layout()
    current_dir = Path(__file__).parent
    plt.savefig(current_dir / "rainbow_grayscale_vs_tint_rewards.png", dpi=200)
    plt.show()